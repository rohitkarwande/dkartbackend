...
export function Login() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Go to homepage - they'll see their name in navbar, can browse marketplace
      navigate("/");
      window.location.href = "/"; // force navbar to re-read token
    } catch (err: any) {
      ...
      <Card className="w-full max-w-md shadow-lg border-0">
        <CardHeader className="space-y-3 text-center mb-4">
          ...
          <CardTitle className="text-2xl font-bold tracking-tight text-slate-900">
            Welcome back to Dr.Kart
          </CardTitle>
          <CardDescription className="text-slate-500">
            {step === "input" ? "Enter your email or mobile to login" : "Enter the 6-digit OTP sent to you"}
          </CardDescription>
        </CardHeader>
        <CardContent>
